...
def add_to_cart(request,product_id):
    ...
    if check_item_presence:
        messages.add_mssage(request,messages.ERROR,'product already present in the cart')
...
def remove_cart(request,cart_id):
    ...
    cart.delete()
    messages.add_message(request,messages.SUCCESS,'product remove from the cart')
    return redirect('/cart')
           ...
           if order.payment_method == 'Cash on Delivery':
             ...
             return redirect('/myorder')
              ...
              return redirect(reverse('esewaform')+"?o_id="+str(order.id)+"&c_id="+str(cart_item.id))
                ...
                return render(request,'userpage/orderform.html',{'form':form})           
...
def esewa_verify(request,order_id,cart_id):
    if request.method == 'GET':
        ...
        if map_data.get('status')=='COMPLETE':
            ...
            order.save()
            cart.delete()
            messages.add_message(request,messages.SUCCESS,'payment sucessfull')
            return redirect('/myorder')
            
        else:
            messages.add_message(request,messages.ERROR,'payment unsucessfull')
            return redirect('/myorder')
...
def update_profile(request):
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            return render(request,'userpage/updateprofile.html',{'forms':form})
